...
@plugin.command(name='doubansub', title='豆瓣片单订阅', desc='输入豆瓣片单id号即订阅影片！', icon='List', run_in_background=True)
def echo(ctx: PluginCommandContext, name: ArgSchema(ArgType.String, '输入纯数字', '请输入豆瓣片单id！')):
    _LOGGER.info(f'豆瓣listId：{name} 加载成功')
    if douban_list_id(name):
        DoubanList_Sub.mb_sub(name)
        return PluginCommandResponse(True, f'豆瓣listId：{name} 运行成功')
    else:
        return PluginCommandResponse(False, f'豆瓣listId：{name} 运行失败')
    
def douban_list_id(doubanlistid):
    if doubanlistid.isdigit():
        return doubanlistid
    else:
        # ValueError
        return False
...
def get_urllist_pagenumber(doubanlist):
    ...
    if html.xpath('//*[@id="content"]/div/div[@class="article"]/div[@class="doulist-filter"]/a/span/text()')[0].strip('()') != []:
        get_urllist_pagenumber = html.xpath('//*[@id="content"]/div/div[@class="article"]/div[@class="doulist-filter"]/a/span/text()')[0].strip('()')
    else:
        quit()
...
def get_urllist(doubanlist):
    url_list = ["https://www.douban.com/doulist/{}/?start={}".format(doubanlist, i * 25) for i in range(get_urllist_pagenumber(doubanlist))]
    get_movie(url_list)
...
class DoubanList_Sub:
    def mb_sub(doubanlist):
        _LOGGER.info(f'豆瓣片单订阅开始')
        if douban_list_id(doubanlist):
            get_urllist(douban_list_id(doubanlist))
        else:
            _LOGGER.info(f'输入错误，请输入数字！')
            return False
        _LOGGER.info(f'豆瓣片单订阅结束')

# 152800811
# DoubanList_Sub.mb_sub(input('输入'))